...
export const checkurl = (text: string) => {
  ...
  const checklist = /^(?:https?:\/\/)?(?:m\.|www\.|)?(?:youtu\.be\/|youtube\.com\/(?:playlist\?list=))((\w|-).+)(?:\S+)?$/;
  const checkmusiclist = /^(?:https?:\/\/)?music\.?(?:youtu\.be\/|youtube\.com\/(?:playlist\?list=))((\w|-).+)(?:\S+)?$/;
  const checkbillboardoo = /^(?:https?:\/\/)wakmusic\.xyz\/player\/((\w|-){11}).+/;
  return {
    video: text.match(checkvideo),
    list: text.match(checklist),
    musiclist: text.match(checkmusiclist),
